...
import { Activity, AlertOctagon, ArrowDownRight, ArrowUpRight, Ban, Calendar, CheckCircle, ChevronDown, DollarSign, Layers, LogOut, Pause, Play, RefreshCw, RotateCcw, Sliders, TrendingUp, UserPlus, Users, XCircle } from 'lucide-react';
        ...
        <div className="min-h-screen bg-[#09090b] text-zinc-100 flex flex-col font-sans">
            ...
            <header className="sticky top-0 z-30 bg-[#0c0c0e]/95 backdrop-blur border-b border-[#27272a] px-3 sm:px-4 lg:px-8 py-2.5 sm:py-3 flex items-center justify-between gap-2">
                <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                    ...
                    <div className="hidden sm:flex items-center gap-2 ml-2 sm:ml-4 pl-2 sm:pl-4 border-l border-zinc-800">
                        ...
                        <div className="flex items-center gap-1.5 bg-zinc-900 border border-[#27272a] px-2.5 py-1 rounded text-xs font-mono shrink-0">
                            ...
                            <span className="text-zinc-200 font-semibold">{botID}</span>
                        </div>
                        ...
                        <div className="hidden lg:flex items-center gap-1.5 bg-zinc-900 border border-[#27272a] px-2.5 py-1 rounded text-xs font-mono shrink-0">
                            ...
                            <span className="text-blue-400 font-bold">{user?.Symbol || 'BTCUSDT'}</span>
                        ...
                        <div className="hidden xl:flex items-center gap-1 bg-zinc-900 border border-[#27272a] px-2.5 py-1 rounded text-[11px] font-mono text-zinc-400 shrink-0">
                            ...
                            <span className="text-zinc-200 truncate max-w-[140px]">{user?.StrategyName || 'EMA-RSI'}</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
                    ...
                    <div
                        className={`flex items-center gap-1 sm:gap-1.5 px-2 sm:px-2.5 py-1 sm:py-1.5 rounded text-[11px] sm:text-xs font-mono font-semibold border cursor-default select-none ${isLive
                            ? 'bg-emerald-950/50 text-emerald-400 border-emerald-800/80'
                            : 'bg-amber-950/50 text-amber-400 border-amber-800/80'
                            }`}
                        title={`Execution Protocol: ${isLive ? 'LIVE' : 'DEMO'} (Configure in Bot Engine Configuration)`}
                    >
                        <span
                            className={`w-1.5 sm:w-2 h-1.5 sm:h-2 rounded-full ${isLive ? 'bg-emerald-500 animate-pulse' : 'bg-amber-400'
                                }`}
                        />
                    ...
                    <div className="relative">
                        ...
                        >
                            ...
                            <span className="hidden md:inline text-zinc-300 font-medium text-xs truncate max-w-[90px]">{user?.Name || 'Ali Wains'}</span>
                        ...
                        {userDropdownOpen && (
                            <div className="absolute right-0 mt-2 w-56 bg-[#121214] border border-[#27272a] rounded-lg shadow-xl py-1.5 z-50 text-xs">
                                <div className="px-3 py-2 border-b border-zinc-800">
                                    <p className="font-semibold text-zinc-200">{user?.Name || 'Ali Wains'}</p>
                                    <p className="text-zinc-500 font-mono text-[10px] truncate">{user?.Email || 'alimalikwayne@gmail.com'}</p>
                                </div>
                                {onNavigateTrades && (
                                    <button
                                        onClick={() => {
            ...
            <main className="flex-1 p-3.5 sm:p-6 lg:p-8 max-w-[1600px] w-full mx-auto space-y-4 sm:space-y-6">
                ...
                <section className="bg-[#121214] border border-[#27272a] rounded-lg p-3 sm:p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3 sm:gap-4 shadow-md">
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:flex md:flex-wrap items-center gap-2 sm:gap-2.5 w-full md:w-auto">
                        ...
                        <button
                            ...
                            className={`flex items-center justify-center gap-2 px-3.5 sm:px-4 py-2.5 sm:py-2 rounded-md font-mono text-xs font-semibold uppercase tracking-wider transition-all shadow-sm col-span-2 sm:col-span-1 md:w-auto ${isTradingActive
                        ...
                        <button
                            ...
                            disabled={loadingAction === 'forceClose' || !position}
                            className={`flex items-center justify-center gap-1.5 px-2.5 sm:px-3 py-2 rounded-md font-mono text-xs border transition-all ${position
                            ...
                            title={position ? 'Immediately market-close current position' : 'No active position to close'}
                        >
                        ...
                        <button
                            ...
                            disabled={loadingAction === 'cancelOrders'}
                            className="flex items-center justify-center gap-1.5 px-2.5 sm:px-3 py-2 rounded-md font-mono text-xs text-zinc-300 bg-zinc-900 border border-[#27272a] hover:bg-zinc-800 hover:text-white transition-colors"
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-2.5 sm:gap-4 pt-1">
                            ...
                            <div className="space-y-1">
                                ...
                                <div className="flex items-center gap-1.5">
                                    <span className="font-bold text-sm font-mono text-white">{position.symbol}</span>
                                    <span
                                        className={`text-[10px] font-mono font-bold px-1.5 py-0.5 rounded border ${position.side === TradeSide.Long || position.side === 0
                                            ? 'text-emerald-400 bg-emerald-950/60 border-emerald-800'
                                            : 'text-rose-400 bg-rose-950/60 border-rose-800'
                                            }`}
                                    >
                                        {position.side === TradeSide.Long || position.side === 0 ? 'LONG' : 'SHORT'}
                                    </span>
                                </div>
                            </div>
                            ...
                            <div className="space-y-1">
                                ...
                                <span className="font-mono text-sm font-semibold text-zinc-200">
                                    ${formatCurrency(position.markPrice || position.entryPrice * 1.002, 2)}
                                </span>
                            </div>
                            ...
                            <div className="space-y-1">
                                ...
                                <div className="flex flex-col">
                                    ...
                                    <span className="font-mono text-[10.5px] text-zinc-500">
                                        ≈ ${formatCurrency(position.notional, 2)}
                                    </span>
                                </div>
                            </div>
                            ...
                            <div className="space-y-1">
                                ...
                                <div className="flex items-center gap-1">
                                    <span className="font-mono text-sm font-semibold text-emerald-400">
                                        {position.marginBuffer ? `${position.marginBuffer.toFixed(1)}%` : '88.4%'}
                                    </span>
                            ...
                            <div className="space-y-1">
                                ...
                                {(() => {
                                    const unPnl = position.unrealizedPnl ?? (position.notional * 0.018);
                                    const isPos = unPnl >= 0;
                                    return (
                                        <span
                            ...
                            <div className="space-y-1">
                                ...
                                {(() => {
                                    const roe = position.roePercent ?? 18.2;
                                    const isPos = roe >= 0;
                                    return (
                                        <span
                        ...
                        <div className="py-8 flex flex-col items-center justify-center text-center space-y-2">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-zinc-500 font-mono mt-0.5">
                                    Listening to Binance {user?.Symbol || 'BTCUSDT'} 1m candles. Strategy: {user?.StrategyName || 'EMA-RSI Momentum Scout'}
                                </p>
                            </div>
                        </div>
                    )}
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-3 sm:mb-4 gap-2">
                            <div>
                                <div className="flex flex-wrap items-center gap-2.5">
                                    <h3 className="text-xs font-mono uppercase tracking-wider font-semibold text-zinc-300">
                                        Binance Commission Breakdown
                                    </h3>

                                    {/* Buttons for "PnL" and "Fee" */}
                                    <div className="inline-flex items-center p-0.5 rounded-md bg-[#09090b] border border-[#27272a]">
                                        <button
                                            type="button"
                                            onClick={() => setBreakdownMetric('pnl')}
                                            className={`px-3 py-1 rounded text-xs font-mono font-semibold transition-all flex items-center gap-1.5 ${breakdownMetric === 'pnl'
                                        ...
                                        <button
                                            type="button"
                                            onClick={() => setBreakdownMetric('fee')}
                                            className={`px-3 py-1 rounded text-xs font-mono font-semibold transition-all flex items-center gap-1.5 ${breakdownMetric === 'fee'
                            ...
                            <div className="flex items-center gap-2 font-mono text-xs">
                                {breakdownMetric === 'pnl' ? (
                                    <>
                                        ...
                                        <span
                                            className={`font-bold ${totalBreakdownPnl >= 0 ? 'text-emerald-400' : 'text-rose-400'
                                                }`}
                                        >
                                            {totalBreakdownPnl >= 0 ? '+' : ''}${formatCurrency(totalBreakdownPnl, 2)} USDT
                                        </span>
                                    </>
                                ) : (
                                    <>
                                        ...
                                        <span className="text-blue-400 font-bold">
                                            ${formatCurrency(totalBreakdownFee, 2)} USDT
                                        </span>
                                        {user?.UnpaidFee !== undefined && user.UnpaidFee > 0 && (
                                            <span className="text-[10px] text-amber-400 bg-amber-950/60 border border-amber-800/80 px-1.5 py-0.5 rounded">
                                                Unpaid: ${formatCurrency(user.UnpaidFee, 2)}
                                            </span>
                                        )}
                                    </>
                                )}
                                ...
                                <ResponsiveContainer width="100%" height="100%">
                                    <BarChart data={dailyBreakdownData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                                        ...
                                        <Tooltip
                                            ...
                                            itemStyle={{ color: breakdownMetric === 'pnl' ? '#34d399' : '#93c5fd' }}
                                            labelStyle={{ color: '#a1a1aa' }}
                                            formatter={(value: any) => [
                                                `${breakdownMetric === 'pnl' && Number(value) >= 0 ? '+' : ''}$${value} USDT`,
                                                breakdownMetric === 'pnl' ? 'Realized PnL' : 'Exchange Fee',
                                            ]}
                                        />
                                        {breakdownMetric === 'pnl' ? (
                                            <Bar dataKey="pnl" radius={[3, 3, 0, 0]}>
                                                {dailyBreakdownData.map((entry, index) => (
                                                    <Cell
                                                        key={`pnl-cell-${index}`}
                                            ...
                                            <Bar dataKey="fee" fill="#3b82f6" radius={[3, 3, 0, 0]} />
                                        )}
                                    </BarChart>
                                </ResponsiveContainer>
                            ) : (
                                <div className="h-full flex items-center justify-center text-zinc-600 text-xs font-mono">
                                    No {breakdownMetric === 'pnl' ? 'PnL' : 'fee'} records available
                                </div>
                            )}
                        </div>
                    </div>
                ...
                {/* 5. Log Terminal Area */}
                <section>
                    <LogTerminal logs={logs} onClear={handleClearLogs} isLoading={refreshing} />
                </section>
                            ...
                            <tbody className="divide-y divide-[#27272a]/60">
                                {trades.length === 0 ? (
                                    ...
                                    trades.slice(0, 15).map((t) => {
                                        const isLong = t.side === TradeSide.Long || t.side === 0;
                                        const gross = t.realizedProfit ?? 0;
                                        const fee = t.fee ?? (gross > 0 ? gross * 0.20 : 0);
                                        const comm = t.commission ?? 0;
                                        const net = t.netProfit ?? (gross - fee - comm);
                                        const isNetProfit = net >= 0;

                                        const formattedDate = t.closeTime
                                            ? new Date(t.closeTime).toLocaleString('en-US', {
                                                month: 'short',
                                                day: 'numeric',
                                                hour: '2-digit',
                                                minute: '2-digit',
                                        ...
                                        return (
                                            <tr key={t.uid || t.orderId} className="hover:bg-zinc-900/50 transition-colors">
                                                ...
                                                <td className="py-3 px-3 text-right font-bold font-mono">
                                                    <span className={isNetProfit ? 'text-emerald-400' : 'text-rose-400'}>
                                                        {isNetProfit ? '+' : ''}${formatCurrency(net, 2)}
                                                    </span>
                                                </td>
                                                <td className="py-3 px-3">
